...
public class PendingRequest extends BaseTest {
	...
	public void testPendingRequest() throws Exception {
		logger.info("Starting test for Pending Request");
		test.log(Status.INFO, "Starting Pending Request Test");
		...
		try {
			for (int i = startRow; i <= endRow; i++) {
				...
				test.log(Status.PASS, "Pending Request Test completed for row: " + i);
			}
		} catch (Exception ex) {
			logger.error("Error occurred in Pending Request Test: " + ex);
			test.log(Status.FAIL, "Error occurred in Pending Request Test: " + ex.getMessage());
			throw ex;
		}
	}
}
